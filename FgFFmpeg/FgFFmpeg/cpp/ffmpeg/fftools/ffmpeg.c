...
// AS by lvaj 2020-02-11 for ProgressCallback
static FFmpegProgressCB g_progressCb;
int64_t g_taskId = 0;
// AE by lvaj 2020-02-11 for ProgressCallback
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time)
{
    ...
    for (i = 0; i < nb_output_streams; i++) {
        ...
        if (!vid && enc->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            av_bprintf(&buf_script, "stream_%d_%d_q=%.1f\n",
                       ost->file_index, ost->index, q);
            if (g_progressCb != NULL) {
                g_progressCb(g_taskId, 1, frame_number, ost->st->duration);
            }
            if (is_last_report)
                av_bprintf(&buf, "L");
            if (qp_hist) {
...
int ffmpeg_main(int argc, char **argv, FFmpegLogCB cbLog, FFmpegProgressCB cbProgress, int64_t taskId)
{
    ...
    // AS by lvaj 2020-02-11 for ProgressCallback
    g_progressCb = cbProgress;
    g_taskId = taskId;
    av_log_set_callback(cbLog);
    // AE by lvaj 2020-02-11 for ProgressCallback
    ...
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1])
        ffmpeg_exit_program(69);

    ffmpeg_exit_program(received_nb_signals ? 255 : main_return_code);
